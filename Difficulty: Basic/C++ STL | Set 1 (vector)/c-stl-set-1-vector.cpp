...
void add_to_vector(vector<int> &A, int x)
{
    // Insert x at the back of the vector using push_back
    A.push_back(x);
}
...
void sort_vector_asc(vector<int> &A)
{
    // Sort the vector A in ascending order using the sort function
    sort(A.begin(), A.end());
}
...
void reverse_vector(vector<int> &A)
{
    // Reverse the vector A manually using a two-pointer approach
    int n = A.size();
    int i = 0;
    int j = n - 1;
    while (i < j) {
...
int size_of_vector(vector<int> &A)
{
    // Return the size of the vector using the size() method
    return A.size();
}
...
void sort_vector_desc(vector<int> &A)
{
    // Sort the vector A in descending order using the sort function and greater comparator
...
void print_vector(vector<int> &A)
{
    // Loop through the vector and print each element space-separated
