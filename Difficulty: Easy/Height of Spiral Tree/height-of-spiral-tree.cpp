...
// Function to return the height of the given special binary tree
int findTreeHeight(Node* root) {
    if (root == NULL) {
        return 0;  // If the node is NULL, return height 0
    }
    
    // Special condition: if the left child exists and its right child points to the current node
    if (root->left != NULL && root->left->right == root) {
        return 0;  // Return height 0 for this case
    }
    ...
    // Return the height of the current node (1 + maximum of left and right subtree heights)
