...
public:
    ...
    {
        // Create an unordered set to store unique values
        unordered_set<int> store = {head->data};

        Node* cur = head->next;
        
        // Traverse the list starting from the second node
        while (cur) {
            // If the current node's data is not in the set, add it
            if (store.find(cur->data) == store.end()) {
                store.insert(cur->data);
            }
            else {
                // If the data is already in the set, remove the node
                cur->prev->next = cur->next;  // Update previous node's next pointer
                if (cur->next)                // If there's a next node, update its prev pointer
