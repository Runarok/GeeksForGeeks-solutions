...
class Solution {
    reverseLevelOrder(root) {
        ...
        let queue = [root]; // Use a queue for level-order traversal
        let stack = []; // Use a stack to store elements in reverse order

        while (queue.length > 0) {
            let node = queue.shift(); // Remove from front of queue
            stack.push(node.data); // Store node value in stack

            // Add right child first, then left (to maintain reverse order)
            if (node.right) queue.push(node.right);
            if (node.left) queue.push(node.left);
        }

        return stack.reverse(); // Reverse stack to get final answer
    }
}
